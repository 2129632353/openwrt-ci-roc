...
if package_enabled tailscale luci-app-tailscale-community; then
  ...
  sed -i 's/^PKG_VERSION:=.*$/PKG_VERSION:=1.102.2/' package/tailscale/Makefile
  sed -i 's/^PKG_HASH:=.*$/PKG_HASH:=66c447b5555aa89e0690dc21e0ec59421302ceaa982f3ccc0df686b16f5d5505/' package/tailscale/Makefile
  grep -q '^PKG_VERSION:=1.102.2$' package/tailscale/Makefile || { echo "Error: tailscale version pin failed" >&2; exit 1; }
  grep -q '^PKG_HASH:=66c447b5555aa89e0690dc21e0ec59421302ceaa982f3ccc0df686b16f5d5505$' package/tailscale/Makefile || { echo "Error: tailscale hash pin failed" >&2; exit 1; }
fi
